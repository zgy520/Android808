...
public class Bcd {
     ...
     * @return
     */
    public static byte[] fromOldDateTime(Date date){
        byte[] bcd = new byte[6];
        bcd[0] = bcd(date.getYear() % 100);
        bcd[1] = bcd(date.getMonth());
        bcd[2] = bcd(date.getDate());
        bcd[3] = bcd(date.getHours());
        bcd[4] = bcd(date.getMinutes());
        bcd[5] = bcd(date.getSeconds());
        return bcd;
    }
     ...
     * @return
     */
    public static Date toOldDateTime(byte[] bcd){
        ...
        if (year < YEAR_RANGE)
            year += 100;
        try {
            return simpleDateFormat.parse(String.format("%d-%d-%d %d:%d:%d",year,num(bcd[i-4]),num(bcd[i - 3]),
                    num(bcd[i - 2]),
                    num(bcd[i - 1]),
                    num(bcd[i])));
